...
const SearchBar = ({
  ...
  const [selectedCategory, setSelectedCategory] = useState(
    categories[0] || (isRTL ? "جميع المنتجات" : "All Products")
  );
  ...
  return (
    <div
      className={`flex items-center justify-between w-full max-w-5xl mx-auto py-2 ${className} ${
        ...
        >
          ...
          <ChevronDown
            className={`w-4 h-4 transition-transform duration-200 ${
              isDropdownOpen ? "rotate-180 text-teal-600" : "text-gray-600"
            }`}
          />
        </button>
              ...
              <button
                ...
                className={`w-full px-4 py-2 hover:bg-gray-50 transition-colors duration-150 text-sm first:rounded-t-lg last:rounded-b-lg ${
                  category === selectedCategory
                    ? "bg-gray-50 text-teal-600 font-medium"
                    : "text-gray-700"
      ...
      <div className="relative" ref={sortRef}>
        ...
        >
          <Funnel
            ...
            ${isSortOpen || selectedSort ? "text-teal-600" : "text-gray-600"}`}
          />
          <span
            className={`text-sm font-semibold 
            ${isSortOpen || selectedSort ? "text-teal-600" : "text-gray-800"}
            ${isRTL ? "font-cairo" : ""}`}
          >
            {selectedSort || sortByLabel}
          </span>
        </button>

        {isSortOpen && (
          <div
            className={`absolute top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-50 min-w-[120px] ${
